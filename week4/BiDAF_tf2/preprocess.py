...
BERT_EMB_PATH = r"data/glove/glove.6B.50d.txt"
...
class Preprocessor:
    def __init__(self, datasets_fp, max_length=384, stride=128):
        ...
        # self.charset = set()
        # self.wordset = set()
        self.w2id, self.id2w = {'pad':0, 'cls':1, 'sep':2,'unk':3},{}
    ...
    def get_id2word(self):
        # 翻转word2id获得id2word
        self.id2w = dict(zip(self.w2id.values(),self.w2id.keys()))
    ...
    def get_id_word(self, ch):
        if ch not in self.w2id:
            self.w2id[ch] = len(self.w2id)
        return self.w2id.get(ch, self.w2id['unk'])

    def get_dataset(self, ds_fp):
        ccs, qcs, cws, qws, be = [], [], [], [], []
        for _,  cw, qw, b, e in self.get_data(ds_fp):
            # ccs.append(cc)
            # qcs.append(qc)
        ...
        return map(np.array, (cws, qws, be))
    ...
    def get_data(self, ds_fp):
        ...
        for qid, context, question, text, answer_start in self.iter_cqa(dataset):
            ...
            if ne == -1:
                b = e = 0
            yield qid, c_word_ids, q_word_ids, b, e
    ...
    def get_sent_ids_word(self, maxlen=0, begin=False, end=True, word_list=[]):
        return self.convert2id_word(maxlen=maxlen, begin=False, end=True, word_list=word_list)
